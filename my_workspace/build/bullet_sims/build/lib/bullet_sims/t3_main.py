...
class Talos(Robot):
    def __init__(self, simulator, q=None, verbose=True, useFixedBase=True):
        ...
        self.wrapper = pin.RobotWrapper.BuildFromURDF(self.urdf_file_path, self.mesh_directory_path, None, True, None)
...
class CartesianSpaceController:
    ...
    def __init__(self, robot_model, target_link_name, Kp, Kd):
        ...
        if robot_model._use_fixed_base:
            ...
            self.posture_reference = np.zeros(robot_model.actuatedJointCount())
...
class Environment(Node):
    ...
    def update(self, simulation_time, time_step):
        ...
        control_torques = np.zeros(self.robot.actuatedJointCount())
        ...
        if self.current_control_mode == State.JOINT_SPLINE:
            if simulation_time <= self.trajectory_duration:
                ...
                self.current_control_mode = State.CARTESIAN_HOLD
        ...
        elif self.current_control_mode == State.CARTESIAN_HOLD:
