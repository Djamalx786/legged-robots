...
def pendulum_continous_dynamics(x, u):
    ...
    q = x[0]  # angle of the pendulum
    q_dot = x[1]  # angular velocity of the pendulum

    q_ddot = (u - b*q_dot - m*g*l*d.sin(q)) / (m*l**2)  
...
def pendulum_discretized_dynamics(x, u, x_next, dt):
    ...
    # Compute the continuous dynamics at the current state
    x_dot = pendulum_continous_dynamics(x, u)
    
    # Use Euler integration: x(t+dt) = x(t) + dt * x_dot(t)
    x_integrated = x + dt * x_dot
    
    # Return the residual between the Euler integration and x_next
    # The solver will drive this to zero to satisfy dynamics constraints
    residuals = x_integrated - x_next
    return residuals
...
# Add some limits on the control torque between some min and max value
u_min = -1.0  # Minimum torque [Nm]
u_max = 1.0   # Maximum torque [Nm]
