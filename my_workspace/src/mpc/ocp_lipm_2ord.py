...
# Create a grid layout: 3x2 for time series plots, then full width for XY plot
gs = fig.add_gridspec(4, 2, height_ratios=[1, 1, 1, 1.2], hspace=0.3, wspace=0.3)

# X-direction plots
ax1 = fig.add_subplot(gs[0, 0])
ax1.plot(t, state_opt[:, 0], 'b-', linewidth=2, label='CoM Position X')
ax1.plot(t, control_opt[:, 0], 'r--', linewidth=2, label='ZMP Position X')
ax1.plot(t, zmp_ref[:, 0], 'g:', linewidth=2, label='ZMP Reference X')
...
ax2.plot(t, state_opt[:, 1], 'b-', linewidth=2, label='CoM Velocity X')
ax2.set_ylabel('Velocity X [m/s]')
ax2.legend()
ax2.grid(True)

ax3 = fig.add_subplot(gs[2, 0])
ax3.plot(t, acc_x, 'b-', linewidth=2, label='CoM Acceleration X')
...
ax4.plot(t, state_opt[:, 2], 'b-', linewidth=2, label='CoM Position Y')
ax4.plot(t, control_opt[:, 1], 'r--', linewidth=2, label='ZMP Position Y')
ax4.plot(t, zmp_ref[:, 1], 'g:', linewidth=2, label='ZMP Reference Y')
ax4.set_ylabel('Position Y [m]')
ax4.set_title('Y-Direction')
ax4.legend()
ax4.grid(True)

ax5 = fig.add_subplot(gs[1, 1])
ax5.plot(t, state_opt[:, 3], 'b-', linewidth=2, label='CoM Velocity Y')
ax5.set_ylabel('Velocity Y [m/s]')
ax5.legend()
ax5.grid(True)

ax6 = fig.add_subplot(gs[2, 1])
ax6.plot(t, acc_y, 'b-', linewidth=2, label='CoM Acceleration Y')
ax6.set_ylabel('Acceleration Y [m/s²]')
ax6.set_xlabel('Time [s]')
ax6.legend()
ax6.grid(True)

#>>>>TODO: plot everything in xy-plane
# XY-plane plot spanning full width
ax7 = fig.add_subplot(gs[3, :])
plot_foot_steps(foot_steps, footprint, ax7)
ax7.plot(state_opt[:, 0], state_opt[:, 2], 'b-', linewidth=3, label='CoM Trajectory')
ax7.plot(control_opt[:, 0], control_opt[:, 1], 'r--', linewidth=2, label='ZMP Trajectory')
ax7.plot(zmp_ref[:, 0], zmp_ref[:, 1], 'g:', linewidth=2, label='ZMP Reference')
ax7.plot(state_opt[0, 0], state_opt[0, 2], 'go', markersize=8, label='Start')
ax7.plot(state_opt[-1, 0], state_opt[-1, 2], 'ro', markersize=8, label='End')
ax7.set_xlabel('X Position [m]')
ax7.set_ylabel('Y Position [m]')
ax7.set_title('Walking Pattern - XY Plane View')
ax7.legend()
