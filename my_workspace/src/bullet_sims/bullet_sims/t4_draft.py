...
from config import q_home as q_home_conf
...
class Environment(Node):
    ...
    def update(self, simulation_time, time_step):
        ...
        # 1) Update Pinocchio & wrapper state
        self.robot.update()

        # 2) Publish TF: world → base_link
        t = TransformStamped()
        t.header.stamp = self.get_clock().now().to_msg()
        t.header.frame_id = 'world'
        t.child_frame_id = 'base_link'
        base_pos  = self.robot._q[0:3]
        base_quat = self.robot._q[3:7]
        t.transform.translation.x = base_pos[0]
        t.transform.translation.y = base_pos[1]
        t.transform.translation.z = base_pos[2]
        t.transform.rotation.x = base_quat[0]
        t.transform.rotation.y = base_quat[1]
        t.transform.rotation.z = base_quat[2]
        t.transform.rotation.w = base_quat[3]
...
def main():
    """
    Main function implementing TSID-based standing controller
    """
    # Initialize ROS
    rclpy.init()
    
    # Create a minimal ROS node for publishing
    node = rclpy.create_node('tsid_standing_controller')
    ...
    # Initialize TSID wrapper
    tsid_wrapper = TSIDWrapper()
    
    # Initialize PyBullet simulator  
    simulator = PybulletWrapper()
    
    # Create Talos robot with floating base and home configuration
    robot = Talos(
        simulator,
        q=q_home_conf,  # Use configuration from conf module
        verbose=True,
        useFixedBase=False  # Floating base as required
    )
    
    # Inject ROS components into robot
    robot.joint_state_publisher = joint_state_publisher
    robot.node = node
    
    # Main simulation loop
    try:
        publish_counter = 0
        publish_frequency = 30  # 30 Hz as specified
        step_time = simulator.stepTime()
        publish_every_n_steps = int(1.0 / (publish_frequency * step_time))
        ...
        while True:
            ...
            # Update TSID controller - this solves Eq.(10) and returns torques
            control_torques, joint_accelerations = tsid_wrapper.update(
                robot._q,
                robot._v,
                robot.model,
                robot.data
            ...
            # Apply computed torques to robot
            robot.setActuatedJointTorques(control_torques)
            
            # Publish robot state at 30 Hz
            if publish_counter % publish_every_n_steps == 0:
                # Publish joint states
                robot.publish()
                
                # Get and publish base-to-world transformation from TSID
                base_state = tsid_wrapper.baseState()  # Get T_b_w transformation
                ...
                # Extract position and orientation from base state
                t.transform.translation.x = base_state.translation[0]
                t.transform.translation.y = base_state.translation[1] 
                t.transform.translation.z = base_state.translation[2]
                
                # Convert rotation matrix to quaternion
                quaternion = pin.Quaternion(base_state.rotation)
